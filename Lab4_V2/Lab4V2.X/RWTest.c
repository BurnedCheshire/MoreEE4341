...
#define LED3 TRISDbits.TRISD2  // visual feedback about SD usage status - fail
#define LED2 TRISDbits.TRISD1  // visual feedback about SD usage status - pass
   ...
   for(i = 0; i < 16; i++)
    {
        for(j=0; j < N_BLOCKS; j++)
        {
            if(!readSECTOR(addr+(j*i), buffer))	//reading one block at a time
            {
